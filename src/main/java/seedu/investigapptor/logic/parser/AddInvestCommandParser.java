...
/**
 * Parses input arguments and creates a new AddInvestCommand object
 */
public class AddInvestCommandParser implements Parser<AddInvestCommand> {

    /**
     * Parses the given {@code String} of arguments in the context of the AddInvestCommand
     * and returns an AddInvestCommand object for execution.
     * @throws ParseException if the user input does not conform the expected format
     */
    public AddInvestCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL, PREFIX_ADDRESS, PREFIX_TAG);

        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_ADDRESS, PREFIX_PHONE, PREFIX_EMAIL)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddInvestCommand.MESSAGE_USAGE));
        }
        ...
        try {
            ...
            return new AddInvestCommand(person);
